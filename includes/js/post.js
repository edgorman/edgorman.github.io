  ...
  for (index in config_data['content']){
    ...
    switch(key) {
      ...
      case "i": // image
        ...
        break;
      case "f": // iframe
        $('#pagecontent').append("<iframe id=\"f" + index + "\" style=\"width: 100%; border: none;\" src=\"" + con + "\"></iframe><script>window.onresize = function(){$('#f"+index+"').height(document.getElementById('f"+index+"').contentWindow.document.body.children[1].children[0].scrollHeight + 69);}; setTimeout(function(){$('#f"+index+"').height(document.getElementById('f"+index+"').contentWindow.document.body.children[1].children[0].scrollHeight + 69);}, 3000)</script>");
        break;
      case "c": // pseudocode
        console.log("Error: Code not implemented yet: " + key);
        break;
      default:
