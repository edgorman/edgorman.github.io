...
$( document ).ready(function() {
    ...
    // If directory contains index.md, cat file
    var startFiles = commands.ls(terminal, startPath)[2];
    for (let i = 0; i < startFiles.length; i++)
    {
        if (startFiles[i]["_name"] == "index.md")
        {
            window.cat(startFiles[i]["_parent"] + startFiles[i]["_name"])
            break;
        }
    }
});
