...
export function generateContentDirectory(elem, path, files, keepSearch=false){
    if (!keepSearch){
        ...
        $(elem + ' .col-xl-4').append(`<div class="nav-bg mb-3 pt-3 pb-3 pl-3 pr-3" style="cursor:pointer;" data-toggle="collapse" href="#collapseHelpSidebar" role="button" aria-expanded="false" aria-controls="collapseHelpSidebar"></div>`);
        $(elem + ' .nav-bg').append(`<div class="d-flex justify-content-between"><b><i class="fas fa-question-circle"></i> How to use this site</b><i class="fas fa-chevron-down pt-2"></i></div>`);
        ...
        $(elem + ' .col-xl-4').append(`<div class="nav-bg mb-3 pt-3 pb-3 pl-2 pr-2"><form class="form-inline d-flex justify-content-center" id="directory-search"></form></div>`);
        $(elem + ' .nav-bg form').append(`<input class="form-control mr-2" type="search" placeholder="Search this directory..." aria-label="Search" id="filterEntriesInput" onkeyup="window.ls_search();">`);
